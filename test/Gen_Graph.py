...
# if Graphs directory exists then remove it
...
os.system('cp ' + graph_dir + '/*.gfa  ' + graph_dir + '/C4-CHM13_mg.gfa')
...
for h, haplotypes in fasta_gfa_2:
    ...
    ref_walk = ""
    ref_id = ""
    cmd = 'vg convert -g -f ' + graph + ' -W | grep   "P" | head -n 1'
    out = subprocess.check_output(cmd, shell=True)
    out = out.decode("utf-8")
    out = out.split('\t')
    ref_id = out[1]
    ...
    def process_fasta(fasta):
        ...
        cmd = "minichain -t1 -cx asm --vc " + graph + " " + fasta + " > " +  fasta.split('.fa')[0] + ".gaf"
        ...
        with open(gaf, 'r') as f:
            for line in f:
                ...
                if i == 0:
                    ...
                    walks.append(id)
                    walk_ = ref_walk
                    w_lines.append("W\t" + id + "\t1\t" + "C4" + "\t1\t1\t" + walk_ + "\n")
                    i += 1
                if ref_id == field[0]: # if reference then just push exact walk
                    ...
                    walks.append(id)
                    walk = field[5]
                    w_lines.append("W\t" + id + "\t1\t" + "C4" + "\t1\t1\t" + walk + "\n")
